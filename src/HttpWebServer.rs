...
impl HttpWebServer
{
    ...
    /// 
    /// # Arguments
    /// * `msg_from_client` - The message the client has sent us. This will usually be an HTTP GET request that we will parse to get the wanted data.
    ...
    fn handle_get_request(&self, msg_from_client: String, stream: &mut TcpStream)
    {
        ...
        // After I read the file data I make a response with the wanted headers. I turn into a byte vector to append it the file data which is a byte vector as well.
        let mut file_data = fs::read(format!("htmlFiles\\{}", file_name)).unwrap();
    ...
    /// 
    /// # Arguments
    /// * `msg_from_client` - This is the message the client sent. As this is a helper function for the `handle_get_request` this will usually be an HTTP GET request message
